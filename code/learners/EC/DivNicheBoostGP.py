"""
Augmentation of div niche gp to apply boosting. 
"""
...
def divnicheboostgp_member_generation(X,y, params, seed):
    random.seed(seed)
    # unpack parameters
    ...
    # Initalise weight vector for sampling. Begins uniform
    sample_weight = np.ones(len(y)) * 1/len(y)
    ...
    for gen in range(1, ngen + 1):
        ...
        idx = np.random.choice(np.arange(len(X)), batch_size, replace=True, p=sample_weight) # boosting samples different here 
        ...
        er = 1 - acc # issue here 
        print(er)
        norm_er = (er - np.min(er)) / (np.max(er) - np.min(er))
        sample_weight = np.array(norm_er)
